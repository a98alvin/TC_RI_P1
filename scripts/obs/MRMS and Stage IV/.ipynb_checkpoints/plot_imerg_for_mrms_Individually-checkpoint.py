...
def imerg_for_mrms_indiv(start_str,end_str,desired_lon,desired_lat,problematic_times_arr):
    ...
    # If day directory does not exist, make day directory
        ...
        i_recorded_list.append(i) # Keep track of which datetimes are used
    ...
    precip_array = np.asarray(imerg_total_precip)

    da = xr.DataArray(
        data=precip_array,
        dims=["time","lon", "lat"],
        coords=dict(time=(["time"],i_recorded_list),
            lon=(["lon", "lat"], sliced_lon_grid),
            lat=(["lon", "lat"], sliced_lat_grid),
            time_period='start = ' + start_str + ', end = ' + end_str,
        ),
        attrs=dict(
    ...
    if os.path.exists('/Users/acheung/data/intermediates/imerg_individual/start = ' + start_str + ', end = ' + end_str + ' imerg.nc')==True:
        os.remove('/Users/acheung/data/intermediates/imerg_individual/start = ' + start_str + ', end = ' + end_str + ' imerg.nc')
    da.to_netcdf('/Users/acheung/data/intermediates/imerg_individual/start = ' + start_str + ', end = ' + end_str + ' imerg.nc')